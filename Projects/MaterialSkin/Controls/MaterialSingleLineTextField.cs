...
namespace MaterialSkin.Controls
{
    public class MaterialSingleLineTextField : Control, IMaterialControl
    {
        ...
        public MaterialSingleLineTextField()
        {
            ...
            baseTextBox.TextChanged += OnTextChanged;
            baseTextBox.KeyPress += OnKeyPress;
            baseTextBox.KeyUp += OnKeyUp;
        }
        ...
        protected override void OnPaint(PaintEventArgs pevent)
        {
            ...
            if (error)
            {
                Brush brush = new SolidBrush(SkinManager.GetErrorColor());

                g.FillRectangle(brush, baseTextBox.Location.X, lineY, baseTextBox.Width, 1);
            }

            if (!animationManager.IsAnimating())
            {
                //No animation
                Brush brush = baseTextBox.Focused ? SkinManager.ColorScheme.PrimaryBrush : SkinManager.GetDividersBrush();

                if (error)
                    brush = new SolidBrush(SkinManager.GetErrorColor());

                g.FillRectangle(brush, baseTextBox.Location.X, lineY, baseTextBox.Width, baseTextBox.Focused ? 2 : 1);
            }
            else
            {
                //Animate
                ...
                Brush brush = SkinManager.ColorScheme.PrimaryBrush;
                if (error)
                    brush = new SolidBrush(SkinManager.GetErrorColor());

                g.FillRectangle(brush, animationStart - halfAnimationWidth, lineY, animationWidth, 2);
            }
        }
        ...
        private class BaseTextBox : TextBox
        {
            ...
            {
                switch ((string)toolStripItemClickedEventArgs.ClickedItem.Tag)
                {
                    case "Undo":
                        Undo();
                        break;
        ...
        {
            public readonly ToolStripItem undo = new MaterialToolStripMenuItem { Text = "Undo", Tag = "Undo" };
            public readonly ToolStripItem seperator1 = new ToolStripSeparator();
            public readonly ToolStripItem cut = new MaterialToolStripMenuItem { Text = "Cut", Tag = "Cut" };
            public readonly ToolStripItem copy = new MaterialToolStripMenuItem { Text = "Copy", Tag = "Copy" };
            public readonly ToolStripItem paste = new MaterialToolStripMenuItem { Text = "Paste", Tag = "Paste" };
            public readonly ToolStripItem delete = new MaterialToolStripMenuItem { Text = "Delete", Tag = "Delete" };
            public readonly ToolStripItem seperator2 = new ToolStripSeparator();
            public readonly ToolStripItem selectAll = new MaterialToolStripMenuItem { Text = "Select All", Tag = "Select All" };
